...
with open (filename,'r') as fi:
	...
	if fluType=="B":
		...
		#Move file
		command = "cp "+ sys.argv[2] + "/assemblies/rename/"+bname+".fa " + " "  + sys.argv[2] +"/assemblies/rename/type/FLUB/"
		process = subprocess.Popen(command.split(), stdout=subprocess.PIPE)
		output, error = process.communicate()
	elif fluType=="A":
		bname = filename.split("/")[-2]
		...
		output, error = process.communicate()
		#Move file
		command = "cp "+ sys.argv[2] + "/assemblies/rename/"+bname+".fa " + sys.argv[2] +"/assemblies/rename/type/FLUA/"+subtypeHA+subtypeNA+"/"
		process = subprocess.Popen(command.split(), stdout=subprocess.PIPE)
		output, error = process.communicate()
